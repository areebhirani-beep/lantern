...
export function AuthForm({
  ...
  // Inline "forgot password" affordance (signin mode only).
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Registration: the user is already signed in. Surface the verification
      // nudge instead of redirecting — verification is additive, never a gate.
      if (mode === "register") {
        setRegistered(true);
        return;
      }
  ...
  async function sendResetLink() {
    ...
    try {
      ...
      // Swallow: we always show the same neutral confirmation regardless.
    } finally {
      setResetBusy(false);
      setShowReset(false);
      setResetSent(true);
  ...
  // Post-registration confirmation. The user is signed in; this just nudges
  // them to verify and lets them continue at their own pace.
  if (registered) {
    return (
      <div className="w-full text-center" role="status" aria-live="polite">
        <span className="mx-auto mb-4 flex h-11 w-11 items-center justify-center rounded-full bg-pounamu/15 text-pounamu">
          <CheckGlyph />
        </span>
        <h2 className="font-display text-xl text-cream">You&rsquo;re in</h2>
        <p className="mx-auto mt-1.5 max-w-[18rem] text-sm leading-relaxed text-muted">
          Check your email to verify your address. You can keep going in the
          meantime — nothing&rsquo;s blocked.
        </p>
        <button
          type="button"
          onClick={() => {
          ...
          className="mt-5 h-11 w-full rounded-full bg-ember text-sm font-medium text-ink transition-transform hover:scale-[1.01]"
        >
          Continue
        </button>
      </div>
          ...
          <div className="pt-0.5">
            {!showReset && !resetSent && (
              <button
                ...
                className="font-mono text-[11px] uppercase tracking-[0.14em] text-faint transition-colors hover:text-muted"
              >
                Forgot password?
              </button>
            )}
            ...
            {showReset && !resetSent && (
              <div className="space-y-2.5 rounded-data border border-line bg-ink/40 p-3.5">
                <label className="block">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        if (EMAIL_RE.test(resetEmail)) sendResetLink();
                      }
                    }}
                    placeholder="you@example.com"
                    autoComplete="email"
                    className="h-10 w-full rounded-data border border-line bg-ink px-3 text-sm text-cream placeholder:text-faint/70 outline-none transition-colors focus:border-ember/60"
                  />
                </label>
                <button
                  type="button"
                  onClick={sendResetLink}
                  disabled={resetBusy || !EMAIL_RE.test(resetEmail)}
                  className="h-10 w-full rounded-full border border-ember/50 text-sm font-medium text-ember transition-colors hover:bg-ember/10 disabled:opacity-50"
                >
            ...
            {resetSent && (
              <p
                className="font-mono text-xs text-pounamu"
                role="status"
                aria-live="polite"
              >
