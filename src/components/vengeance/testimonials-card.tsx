    ...
    return (
        <div className={cn("flex items-center justify-center p-8", className)}>
            ...
            >
                {/* Counter */}
                {showCounter && (
                    <div className="row-start-1 md:col-start-2 md:row-start-1 text-right font-mono text-sm text-faint">
                    ...
                    <AnimatePresence custom={direction}>
                        {items.map((item, index) => {
                            ...
                            return (
                                <motion.div
                                    key={item.id}
                                    className="absolute inset-0 w-full h-full overflow-hidden border-[6px] bg-surface border-line shadow-2xl rounded-lg"
                                    initial={{
                                        x: offset * 15,
                                        y: Math.abs(offset) * 6,
                ...
                {showNavigation && items.length > 1 && (
                    <div className="col-start-1 md:col-start-2 md:row-start-3 flex gap-2  m-auto -mt-2 md:mt-4  md:m-0">
                        <button
                            ...
                            onClick={handlePrev}
                            className={cn(
                                "flex items-center justify-center w-10 h-10 rounded-full border border-line bg-surface transition-all",
                                activeIndex === 0
